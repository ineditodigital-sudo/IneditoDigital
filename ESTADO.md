...
- **Qué recomienda cada servicio se edita en el panel**: Contenido › Servicios › editar › «Arma tu ruta» (desde el 23-sep, en el lugar de «Servicios relacionados»). Cada renglón: servicio destino, papel y para quién, elegidos de menús, y el porqué. Se guarda en el `data_json` del servicio (`recomendaciones`) y manda sobre lo del código; una lista vaciada a propósito apaga la sección.
- Lo del código, `src/app/data/recomendaciones.ts` (94 razones, con su inglés en el diccionario), queda de respaldo: es lo que se ve mientras un servicio no guarde las suyas, lo que el editor enseña para empezar, y lo único de las páginas de IA y la de posicionamiento en IA, que no son servicios del panel. Una razón nueva escrita en el panel sale en español en la versión en inglés hasta que se traduzca.
- Los textos fijos de la sección, en el panel: Plantilla de página de servicio › «Arma tu ruta».
...
- El campo «Servicios relacionados» salió del panel el 23-sep (el sitio nunca lo mostró y apuntaba a un «email-marketing» que no existe); en su lugar está «Arma tu ruta». Lo viejo se queda en el data_json de cada servicio, sin efecto.
- El editor genérico de listas (`panel/inc/crud.php`, tipo `pares`) ahora acepta menús por columna (`opciones`), anchos (`anchos`), cajas de varias líneas (`largos`), columnas obligatorias (`requeridas`) y un relleno inicial (`respaldo`) para cuando la ficha todavía no tiene esa clave. Pasos y preguntas se ven igual.
...
- **«Arma tu ruta» del código se compila**: el respaldo (`recomendaciones.ts`) llega a los robots y al editor del panel por `dist/datos/recomendaciones.json`, que genera `vite.config.ts`. Si se cambia y no se vuelve a compilar y publicar, se desfasan. Lo que se guarda en el panel no necesita compilar. Un servicio nuevo del panel sale sin la sección hasta que se le agreguen recomendaciones en su editor.
