    ...
    <div className={`flex h-full flex-col border-r border-slate-200 bg-white ${className}`}>
      ...
      <div className="flex items-center gap-2.5 border-b border-slate-100 px-4 py-4">
        ...
        <div className="min-w-0 leading-tight">
          <div className="text-[13px] font-bold tracking-tight text-slate-900">INÉDITO</div>
          <div className="text-[10px] uppercase tracking-[.18em] text-slate-400">Digital</div>
        </div>
      </div>
      ...
      <div className="px-3 pt-3">
        <div className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5">
          <div className="flex items-center justify-between gap-2">
            <span className="truncate text-[12.5px] font-semibold text-slate-900">{EMPRESA.nombre}</span>
            <span className="shrink-0 rounded-full bg-amber-100 px-1.5 py-0.5 text-[9.5px] font-bold uppercase tracking-wider text-amber-800">
              Demo
            </span>
          </div>
          <div className="mt-0.5 text-[11px] leading-snug text-slate-500">{EMPRESA.sector}</div>
        </div>
      </div>

      {/* secciones */}
      <nav className="flex-1 overflow-y-auto px-3 py-3">
        <div className="mb-1.5 px-2 text-[10px] font-semibold uppercase tracking-[.16em] text-slate-400">
          Tu mes
        </div>
              ...
              <li key={id}>
                <button
                  ...
                  className={`relative flex w-full items-center gap-2.5 rounded-lg px-2.5 py-2 text-left text-[13px] transition-colors ${
                    esta ? 'font-semibold text-slate-900' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-800'
                  }`}
                >
                  {esta && (
                    <motion.span
                      layoutId="seccion-activa"
                      className="absolute inset-0 rounded-lg bg-[#7700CE]/[.07]"
                      transition={{ type: 'spring', stiffness: 420, damping: 34 }}
                    />
                  )}
                  <span
                    className="absolute left-0 top-1/2 h-4 w-[3px] -translate-y-1/2 rounded-r transition-opacity"
                    style={{ background: MORADO, opacity: esta ? 1 : 0 }}
                  />
                  <Icono
                    size={15}
                    strokeWidth={1.9}
                    className="relative shrink-0"
                    style={{ color: esta ? MORADO : undefined }}
                  />
      ...
      <div className="border-t border-slate-100 p-3">
        ...
        <p className="mt-2.5 px-1 text-[10.5px] leading-snug text-slate-400">
